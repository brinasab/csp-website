...
This first homework assignment has two main goals:

1. To verify your computer setup and ensure you can submit assignments correctly.  
2. To practice basic R syntax, Markdown, and R Markdown.


# Accessing and Cloning Your `hw01` Repository

::: {.callout-important}
This will work only if you have submitted your GitHub username (see Lecture 1) and accepted the invitation to join our GitHub organization (check the email linked to your GitHub account).

Make sure you’ve completed the setup under [Software Setup](../setup/index.qmd) and confirmed everything is working properly.
:::

**Accessing the repo:** 

1. Go at [this link](https://classroom.github.com/a/KCzauQcd) to accept the invitation and create your private `hw01` repository on GitHub. Your repository will be named `hw01-<USERNAME>` and will be ready within a few seconds.
...
- Add *at least two R code chunks* with a short explanation in plain text (not as code comment):  
  - Your code can be simple, but it must run and produce the correct output 
  - Your final knitted `.html` file should show both code and output
  - If your code generates plots (not required, but you possible), make sure the output images are also pushed to GitHub
...
Your `Rcode.rmd` file must knit successfully. It should contain at least two new YAML header fields and two R code chunks, each introduced with a brief explanation written as plain text (not as code comments). 
