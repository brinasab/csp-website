...
::: {.callout-tip title="**Now we ask: Can our code make decisions?**"}
...
Use `ifelse()` to apply a condition to **each element of a vector** thus we call this a "vectorized" test.  
...
- The input to `ifelse()` is a **vector**, not just a single value, so the output is also a **vector**. The code evaluates **each element of the vector**, applying the specified action for each case.
...
People often use `ifelse()` also to recode variables in a dataframe.
