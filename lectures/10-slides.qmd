...
The previous code prints the updated sum each time, but only saves the final value for sum. To keep all intermediate values, we need to store them in a vector:
...
for (i in c(1,2,3)) {
  ...
  sum <- sum + i
  print(paste("Current sum is:", sum))
  output[i] <- sum     # saves sum in position i
}
...
::: {.callout-note .fragment .small-text title="Why `output[i] <- sum`?"}

When `i = 1`, `sum = 0 + 1 = 1` → store `1` in `output[1]` → output becomes: `1 0 0`  
When `i = 2`, `sum = 1 + 2 = 3` → store `3` in `output[2]` → output becomes: `1 3 0`  
When `i = 3`, `sum = 3 + 3 = 6` → store `6` in `output[3]` → output becomes: `1 3 6`
...
When `i = 4` → `sum = 1 + 4 = 5` → store `5` in `output[4]` → output becomes: `1 0 0 5`

When `i = 6` → `sum = 5 + 6 = 11` → store 11 in `output[6]` → output becomes: `1 0 0 5 NA 11`
...
## 💻 Practice: Fix this For Looop

You are a farmer writing a for loop to track `tomatoes` sold over three days. You want to add up the number of tomatoes sold and store the running total in a vector called `output`.

But something is wrong with the code — can you spot and fix it? First, *think* about what the `output` vector should be, then look at the code and figure out why it's not giving you that.
